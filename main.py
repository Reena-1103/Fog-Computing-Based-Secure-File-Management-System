...
class FogApp:
    def __init__(self, root):
        ...
        self.permissions = {
            ...
            "administrator": ["upload", "view", "download", "decrypt"],
            "company": ["upload", "view"],
            "cloud": ["view"],
            "auditor": ["view"]
        }
    ...
    def decrypt_data(self, iv_b64: str, ct_b64: str) -> bytes:
        def has_permission(self, action):
            role = self.users.get(self.current_user, {}).get("role", "")
    ...
    def login(self):
        ...
        if self.login_attempts.get(username, 0) >= 3:
            messagebox.showerror("Blocked", "Too many attempts. Try later.")
            return
    ...
    def setup_main_ui(self):
        ...
        # Upload button (only if allowed)
        if self.has_permission("upload"):
            tk.Button(nav_frame, text="Upload File", width=12,
                    bg="#2ecc71", fg="white",
                    command=self.file_ui.upload_file).grid(row=0, column=1, padx=4)
        # View button (only if allowed)
        if self.has_permission("view"):
            tk.Button(nav_frame, text="View Files", width=12,
                    bg="#f39c12", fg="white",
                    command=self.file_ui.view_files).grid(row=0, column=2, padx=4)
